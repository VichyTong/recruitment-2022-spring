...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
    ...
    for(int i=0; i<N; i+=VECTOR_WIDTH){
        ...
        _cs149_vadd_float(result, zero_f, x, maskIsNotNegative);
        _cs149_vsub_int(count, y, one_i, maskIsNotNegative);
        ...
        while(done != VECTOR_WIDTH){
            ...
            _cs149_veq_int(maskIsNegative, count, zero_i, maskAll);
            done = _cs149_cntbits(maskIsNegative);
            maskIsNotNegative = _cs149_mask_not(maskIsNegative);
        }
...
float arraySumVector(float* values, int N) {
    ...
    for (int i=0; i<N; i+=VECTOR_WIDTH) {
        maskAll = _cs149_init_ones();
        _cs149_vload_float(x, values+i, maskAll);
        int count = (int)log2(VECTOR_WIDTH);
        while(count--){
            _cs149_hadd_float(x, x);
            _cs149_interleave_float(x, x);
        }
        _cs149_vstore_float(tmp, x, maskAll);
        ans += tmp[0];
    }
    return ans;
}
